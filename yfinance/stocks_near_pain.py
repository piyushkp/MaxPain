...
def get_stocks_around_max_pain(date, symbols):
    ...
    for symbol in symbols:
        ...
        print(count)
    print(tickers)
    return symbol, cp, date, mp, mp_COI, mp_POI


def get_big_strangles(date, symbols):
    for symbol in symbols:
        mp, mp_COI, mp_POI = max_pain(date, symbol)
        if mp is not None:
            if mp_COI + mp_POI > 10000:
...
def get_stock_symbols():
    ...
    symbols = set.union( sym2, sym3, sym4 )
...
if __name__ == "__main__":
    ...
    get_big_strangles(date="2022-01-21", symbols=symbols)